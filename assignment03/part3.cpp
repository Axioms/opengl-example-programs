...
void init(void)
{
    ...
    for (auto x = 0; x < number; ++x)
    {
        for (auto y = 0; y < number; ++y)
        {
            ...
            auto* artf_cyinder_north = new cs4722::artifact_rotating();
            auto* artf_cyinder_east = new cs4722::artifact_rotating();
            auto* artf_cyinder_south = new cs4722::artifact_rotating();
            auto* artf_cyinder_west = new cs4722::artifact_rotating();
            ...
            artf_cyinder_east->world_transform.translate = glm::vec3((base + x * d) + (radius * 1.5), (base + y * d), base);
            artf_cyinder_south->world_transform.translate = glm::vec3((base + x * d), (base + y * d) - (radius * 1.5), base);
            artf_cyinder_west->world_transform.translate = glm::vec3((base + x * d) - (radius * 1.5), (base + y * d), base);
            ...
            artf_cyinder_north->animation_transform.rotation_axis = glm::vec3(0, 1, 0);
            artf_cyinder_east->animation_transform.rotation_axis = glm::vec3(1, 0, 0);
            artf_cyinder_south->animation_transform.rotation_axis = glm::vec3(0, -1, 0);
            artf_cyinder_west->animation_transform.rotation_axis = glm::vec3(-1, 0, 0);


            artf_cyinder_north->animation_transform.rotation_center = glm::vec3((base + x * d), (base + y * d) + (radius * 1.5), base);
            artf_cyinder_east->animation_transform.rotation_center = glm::vec3((base + x * d) + (radius * 2), (base + y * d) - (radius * 1), base);
            artf_cyinder_south->animation_transform.rotation_center = glm::vec3((base + x * d), (base + y * d) - (radius * 1.5), base);
            artf_cyinder_west->animation_transform.rotation_center = glm::vec3((base + x * d) + (radius * 2), (base + y * d) + (radius * 1), base);


            artf_cyinder_north->rotation_rate = 0.4;
            artf_cyinder_east->rotation_rate = 0.8;
            artf_cyinder_south->rotation_rate = 0.6;
            artf_cyinder_west->rotation_rate = 1.0;
...
{
    glfwInit();
    auto *window = cs4722::setup_window("Assignment 3 Part 3", .9);
