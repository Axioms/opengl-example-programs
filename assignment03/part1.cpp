...
void init(void)
{
    ...
    for (auto x = 0; x < number; ++x)
    {
        for (auto y = 0; y < number; ++y)
        {
                ...
                artf_cyinder_north->world_transform.translate = glm::vec3((base + x * d), (base + y * d) + (radius * 2), base);
                artf_cyinder_east->world_transform.translate = glm::vec3((base + x * d) + (radius * 1.5), (base + y * d), base);
                artf_cyinder_south->world_transform.translate = glm::vec3((base + x * d), (base + y * d) - (radius * 2), base);
...
{
    glfwInit();
    auto *window = cs4722::setup_window("Assignment 3 Part 1", .9);
