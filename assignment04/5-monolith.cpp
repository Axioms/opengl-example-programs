...
//    the_view->enable_logging = false;
    ...
    cs4722::shape* m = new cs4722::block();
    ...
    auto* artf_monolith = new cs4722::artifact_rotating();

    artf_monolith->the_shape = (m);
    artf_monolith->world_transform.scale = glm::vec3(pc_height /2, pc_height * 4, pc_height/8);
    artf_monolith->world_transform.translate = glm::normalize(glm::vec3 (1, 0.1, 0.1)) * (asteroid_radius + 2 * pc_height) + asteroid_center;
    artf_monolith->the_shape->color_set_ = std::vector<cs4722::color>({cs4722::x11::gray60, cs4722::x11::gray100});
    artf_monolith->surface_material.diffuse_color = cs4722::x11::gray30;

    auto Pos = artf_monolith->world_transform.translate - asteroid_center;
    artf_monolith->world_transform.rotation_axis = glm::vec3(Pos.z, 0, -Pos.x);
    artf_monolith->world_transform.rotation_angle = glm::acos(artf_monolith->world_transform.translate.y / glm::length(artf_monolith->world_transform.translate));
    artf_monolith->world_transform.rotation_center = glm::vec3(0, 0, 0);
...
//    artf->rotation_rate = (M_PI / 3);
    ...
    artf_list.push_back(artf_monolith);
...
//    auto* view = static_cast<cs4722::view*>(glfwGetWindowUserPointer(window));
    ...
    auto v = glm::normalize(the_view->camera_position - asteroid_center);
    the_view->camera_up = v;

    auto t = glm::dot(v, the_view->camera_left) * v;
    t = glm::normalize(the_view->camera_left - t);
    the_view->camera_left = t;

    the_view->camera_forward = glm::cross(the_view->camera_left, the_view->camera_up);
...
void move_callback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT))
    {
        ...
        auto const rot_lr = glm::mat3(glm::rotate(glm::identity<glm::mat4>(),
                                                  static_cast<float>(-dx * lr_pan_scale), the_view->camera_up/* glm::vec3(0, 1, 0) */));
